...
export default {
  ...
  build({ R, P, M, THREE, store, parts }) {
    ...
    const beltY = 0.8;
    function conveyor(x0, x1, z) { const len = x1 - x0, cx = (x0 + x1) / 2; P.box(len, 0.06, 0.62, belt, cx, beltY, z); P.box(len, 0.14, 0.05, M.steel, cx, beltY - 0.02, z - 0.33); P.box(len, 0.14, 0.05, M.steel, cx, beltY - 0.02, z + 0.33); for (const lx of [x0 + 0.25, x1 - 0.25]) for (const dz of [-0.28, 0.28]) P.box(0.05, beltY - 0.05, 0.05, M.steel, lx, (beltY - 0.05) / 2, z + dz); const motor = P.box(0.22, 0.18, 0.18, M.black, x1 - 0.3, beltY - 0.2, z + 0.45, 0.01); P.box(0.03, 0.22, 0.03, M.black, x1 - 0.3, beltY - 0.03, z + 0.58); const led = P.beacon(x1 - 0.3, beltY + 0.08, z + 0.58); return { x0, x1, z, motor, led }; }
    ...
    // Parcels flow along the whole line as one queue: each advances at its section's speed, never closer than
    // SPACING to the one ahead, and stops at the end of its section when the next section is dead (end stop).
    // Delivered parcels re-enter from receiving. Diverted parcels ride lane B into the tote.
    const SEC = [C.c1, C.c2, C.c3], SPACING = 0.62, END_STOP = 0.22;
    const flow = []; for (let i = 0; i < 13; i++) { const x = -4.8 + i * 0.8; flow.push({ mesh: P.parcel(x, beltY + 0.03, 0.5, 0.26 + (i % 3) * 0.04), x, lane: 'main', z: 0.5, onCart: null }); }
    ...
    function cart(x, z) { const g = new THREE.Group(); g.position.set(x, 0, z); R.scene.add(g); const body = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.22, 0.6), M.steel); body.position.y = 0.2; body.castShadow = true; g.add(body); const colH = beltY - 0.35; const col = new THREE.Mesh(new THREE.BoxGeometry(0.5, colH, 0.4), M.black); col.position.y = 0.31 + colH / 2; col.castShadow = true; g.add(col); const deck = new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.03, 0.56), M.steel); deck.position.y = beltY - 0.015; deck.castShadow = true; g.add(deck); for (const dz of [-0.29, 0.29]) { const rail = new THREE.Mesh(new THREE.BoxGeometry(0.85, 0.05, 0.02), M.yellow); rail.position.set(0, beltY + 0.025, dz); g.add(rail); } for (const [dx, dz] of [[-0.3, -0.25], [0.3, -0.25], [-0.3, 0.25], [0.3, 0.25]]) { const w = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.06, 16), M.rubber); w.rotation.x = Math.PI / 2; w.position.set(dx, 0.08, dz); g.add(w); } const beacon = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.06, 12), new THREE.MeshStandardMaterial({ color: 0x2FBF71, emissive: 0x2FBF71, emissiveIntensity: 1.5 })); beacon.position.set(-0.4, beltY + 0.08, 0.24); g.add(beacon); const load = P.parcel(0, 0, 0, 0.3); R.scene.remove(load); load.position.set(0, beltY + 0.105, 0); g.add(load); load.visible = false; return { group: g, body, beacon, load, wasLoaded: false }; }
    const HOME_A = new THREE.Vector3(-3.0, 0, 2.6), HOME_B = new THREE.Vector3(-1.4, 0, 2.6);
    const A = cart(HOME_A.x, HOME_A.z), B = cart(HOME_B.x, HOME_B.z);
    ...
    const P1 = new THREE.Vector3(-2.35, 0, 1.18), P2 = new THREE.Vector3(1.55, 0, 1.18); // cart shuttle path, deck alongside the belt's front edge
    let lastT = null;
    ...
    return {
      ...
      update(s, t) {
        ...
        // conveyor beacons
        for (const k of ['c1', 'c2', 'c3']) { const c = C[k], st = s[k]; const bad = st.status === 'fault'; c.led.material.color.set(bad ? 0xE0563A : st.running && st.speed > 0.05 ? 0x3AB7FF : 0x2FBF71); c.led.material.emissive.copy(c.led.material.color); c.led.material.emissiveIntensity = bad ? (Math.floor(t * 3) % 2 ? 6 : 0.6) : st.running ? 2.5 : 1.2; c.led.visible = st.status !== 'offline'; }
        // parcel queue
        const secSpeed = (i) => { const st = s[['c1', 'c2', 'c3'][i]]; return st.status === 'fault' || !st.running ? 0 : st.speed * 1.4; };
        const main = flow.filter((p) => p.lane === 'main' && !p.onCart && p.x > -5.5).sort((a, b) => b.x - a.x);
        let ahead = null;
        for (const p of main) {
          const si = secAt(p.x); let nx = p.x + secSpeed(si) * dt;
          if (ahead) nx = Math.min(nx, ahead.x - SPACING);
          if (si < 2 && secSpeed(si + 1) === 0) nx = Math.min(nx, SEC[si].x1 - END_STOP);
          p.x = Math.max(p.x, nx);
          if (si === 1 && s.gate.lane === 'B' && s.gate.pos > 0.9 && p.x >= C.c2.x1 - 0.05) { p.lane = 'B'; p.z = 0.5; p.x = 1.3; continue; }
          if (p.x > C.c3.x1 - 0.25) { p.x = -6; continue; } // delivered at the dock; re-enters from receiving when there is room
          ahead = p;
        }
        for (const p of flow) { if (p.lane === 'B' && !p.onCart) { p.z += secSpeed(2) * dt; if (p.z > 2.95) { p.lane = 'main'; p.z = 0.5; p.x = -6; } } }
        const tail = main.length ? main[main.length - 1].x : Infinity;
        const waiting = flow.find((p) => p.x <= -5.5 && !p.onCart); if (waiting && tail - (-4.95) >= SPACING && secSpeed(0) > 0) waiting.x = -4.95;
        // carts: pick up the parcel waiting at the end of C1, drop it at the start of C3
        for (const [cart, st, home] of [[A, s.agvA, HOME_A], [B, s.agvB, HOME_B]]) {
          if (st.load && !cart.wasLoaded) { const q = flow.filter((p) => p.lane === 'main' && !p.onCart && p.x > C.c1.x1 - 1.2 && p.x <= C.c1.x1).sort((a, b) => b.x - a.x)[0]; if (q) q.onCart = cart; }
          if (!st.load && cart.wasLoaded) { const q = flow.find((p) => p.onCart === cart); if (q) { q.onCart = null; q.lane = 'main'; q.z = 0.5; q.x = C.c3.x0 + 0.3; } }
          cart.wasLoaded = !!st.load; cart.load.visible = !!st.load;
          if (st.state === 'parked') cart.group.position.lerp(home, 0.08); else cart.group.position.lerpVectors(P1, P2, st.u);
          const bad = st.status === 'fault'; cart.beacon.material.color.set(bad ? 0xE0563A : st.state === 'moving' ? 0xFFB020 : 0x2FBF71); cart.beacon.material.emissive.copy(cart.beacon.material.color); cart.beacon.material.emissiveIntensity = st.state === 'moving' || bad ? (Math.floor(t * 4) % 2 ? 4 : 1) : 1.5;
        }
        for (const p of flow) { p.mesh.visible = !p.onCart && p.x > -5.5; if (p.lane === 'B') p.mesh.position.set(1.3, p.mesh.position.y, p.z); else p.mesh.position.set(p.x, p.mesh.position.y, 0.5); }
        // scanner, gate, dock
        ...
        gatePivot.rotation.y = -s.gate.pos * 0.9;
  ...
  prompts: [
    {
      ...
      steps: [
        ...
        { parallel: [{ tween: 'scanner.count', to: 16, ms: 4000 }, { tween: 'dock.parcels', to: 12, ms: 4000 }, { fn: async ({ store, fast }) => { await new Promise((r) => setTimeout(r, fast ? 0 : 800)); if (!store.get('plan.earlyGreen')) store.set('dock.light', 'green'); } }] },
        ...
        { fn: async ({ store, fast }) => { const T = fast ? 0 : 1200; for (let i = 0; i < 3; i++) { store.set('agvA.load', true); store.set('agvB.load', false); await Promise.all([store.tween('agvA.u', 1, T), store.tween('agvB.u', 0, T), store.tween('scanner.count', 16 + (i + 1) * 4, T), store.tween('dock.parcels', 12 + (i + 1) * 4.6, T)]); store.set('agvA.load', false); store.set('agvB.load', true); await Promise.all([store.tween('agvA.u', 0, T), store.tween('agvB.u', 1, T), store.tween('scanner.count', 16 + (i + 1) * 4 + 2, T), store.tween('dock.parcels', 12 + (i + 1) * 4.6 + 2.5, T)]); } } },
        { say: '32 at the dock, carts holding pace. 8 to go.' },
        { fn: async ({ store, fast }) => { const T = fast ? 0 : 1500; await Promise.all([store.tween('scanner.count', 40, T), store.tween('dock.parcels', 40, T), store.tween('agvA.u', 1, T), store.tween('agvB.u', 0, T)]); } },
    ...
    },
    {
      ...
      steps: [
        ...
        { parallel: [{ tween: 'agvA.u', to: 0.5, ms: 1200 }, { tween: 'agvB.u', to: 0.7, ms: 1200 }] },
        { beat: 'run' },
        ...
        { parallel: [{ tween: 'c1.speed', to: 0, ms: 1600 }, { tween: 'c2.speed', to: 0, ms: 1600 }, { tween: 'c3.speed', to: 0, ms: 1600 }] },
